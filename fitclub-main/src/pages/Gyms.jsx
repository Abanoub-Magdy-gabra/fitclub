...
const Gyms = () => {
    ...
    const fetchGyms = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          if (error.message && error.message.includes("relation \"public.gyms\" does not exist")) {
            console.log('Gyms table does not exist in Supabase, using mock data instead');
        ...
        if (data && data.length > 0) {
          ...
          const formattedGyms = data.map(gym => ({
            ...
            coordinates: gym.coordinates ? JSON.parse(gym.coordinates) : { lat: 30.0444, lng: 31.2357 },
            ...
            hours: gym.hours ? JSON.parse(gym.hours) : "Open 24/7",
            amenities: gym.amenities ? JSON.parse(gym.amenities) : ["Weight Room", "Cardio"],
            images: gym.images ? JSON.parse(gym.images) : [
            ...
            description: gym.description || "A modern fitness center with state-of-the-art equipment."
          }));
  ...
  useEffect(() => {
    if (showMap && mapRef.current && !mapInstance.current && filteredGyms.length > 0) {
      initializeMap();
    }
        ...
        const infoWindow = new google.maps.InfoWindow({
          content: `
            <div style="padding: 10px; max-width: 250px;">
              <h3 style="margin: 0 0 5px 0; font-size: 16px; font-weight: bold;">${gym.name}</h3>
              <p style="margin: 0 0 5px 0; color: #666; font-size: 14px;">${gym.address}</p>
              <p style="margin: 0 0 5px 0; font-size: 14px;">Rating: ${gym.rating} ⭐ (${gym.reviewCount} reviews)</p>
              <p style="margin: 0 0 5px 0; font-size: 14px; font-weight: bold; color: #e53e3e;">
                ${gym.monthlyPrice} ${gym.currency}/month
              </p>
              <p style="margin: 0; color: #666; font-size: 12px;">${formatHours(gym.hours)}</p>
            </div>
          `
        });
