    ...
    const fetchPlans = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          if (error.message && error.message.includes("relation \"public.subscription_plans\" does not exist")) {
            console.log('Subscription plans table does not exist in Supabase, using mock data instead');
          ...
          data.forEach(plan => {
            ...
            const planBase = {
              ...
              features: plan.features ? JSON.parse(plan.features) : [
                `Access to ${plan.name === 'Basic' ? '100+' : plan.name === 'Premium' ? '500+' : '1000+'} gyms`,
                "24/7 gym access",
                `${plan.name === 'Basic' ? 'Basic' : plan.name === 'Premium' ? 'Advanced' : 'Elite'} workout plans`,
                `${plan.name === 'Basic' ? 'Standard' : 'All'} equipment access`,
                "Mobile app access"
              ],
              notIncluded: plan.not_included ? JSON.parse(plan.not_included) : [],
